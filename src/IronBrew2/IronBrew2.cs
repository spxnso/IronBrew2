...
using IronBrew2.Obfuscator.ControlFlow;
using IronBrew2.Obfuscator.Encryption;
using IronBrew2.Obfuscator.VM;
...
namespace IronBrew2
{
    public static class IB2
    {
        ...
        public static bool IsLuajitPresent()
        {
            try
            {
        ...
        public static bool Obfuscate(string path, string input, ObfuscationSettings settings, out string error)
        {
            try
            {
                ...
                if (!IsLuajitPresent())
                {
                    Console.Error.WriteLine($"Could not find luajit. Exiting..");
                    Environment.Exit(1);
                }
                    ...
                    StartInfo =
                           {
                               FileName = $"luajit",
                               Arguments =
                                   "../IronBrew2/Lua/Minifier/luasrcdiet.lua --noopt-whitespace --noopt-emptylines --noopt-numbers --noopt-locals --noopt-strings --opt-comments \"" +
                                   input                                                       +
                                   "\" -o \""                                                  + t0 + "\"",
                               UseShellExecute        = false,
                               RedirectStandardError  = true,
                ...
                string c = new VMGenerator(context).GenerateVM(settings);
                    ...
                    StartInfo =
                           {
                               FileName = $"{OS}luajit",
                               Arguments =
                                   "../IronBrew2/Lua/Minifier/luasrcdiet.lua --maximum --opt-entropy --opt-emptylines --opt-eols --opt-numbers --opt-whitespace --opt-locals --noopt-strings \"" +
                                   t2                                                                                                                                                +
                                   "\" -o \"" +
                                    t3 +
                                   "\""
