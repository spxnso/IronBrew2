...
namespace IronBrew2.Obfuscator.VM;

public class VMGenerator
{
    private ObfuscationContext _context;

    public VMGenerator(ObfuscationContext context) =>
        _context = context;
    ...
    public bool IsUsed(Chunk chunk, VOpCode virt)
    {
        ...
        foreach (Instruction ins in chunk.Instructions)
            if (virt.IsInstruction(ins))
            {
                ...
                ins.CustomData = new CustomInstructionData(virt);
                isUsed = true;
            }
    ...
    public void FoldMutations(List<OpMutated> mutations, HashSet<OpMutated> used, Chunk chunk)
    {
        ...
        for (int i = 0; i < chunk.Instructions.Count; i++)
        {
            ...
            foreach (OpMutated mut in mutations)
                if (data is not null && data.OpCode == mut.Mutated && data.WrittenOpCode == null)
                {
    ...
    public List<OpSuperOperator> GenerateSuperOperators(Chunk chunk, int maxSize, int minSize = 5)
    {
        ...
        while (c < chunk.Instructions.Count)
        {
            ...
            OpSuperOperator superOperator = new OpSuperOperator(new VOpCode[targetCount]);
            ...
            if (!d)
            {
                ...
                targetCount = cutoff;
                superOperator = new OpSuperOperator(new VOpCode[targetCount]);
            }
    ...
    public void FoldAdditionalSuperOperators(Chunk chunk, List<OpSuperOperator> operators, ref int folded)
    {
        ...
        while (c < chunk.Instructions.Count)
        {
            ...
            foreach (OpSuperOperator op in operators)
            {
                ...
                {
                    CustomInstructionData? cData;
                    for (int j = 0; j < targetCount; j++)
                    {
                        skip[c + j] = true;
                        cData = chunk.Instructions[c + j].CustomData;
                        if (cData != null)
                        {
                            cData.WrittenOpCode = new OpSuperOperator([]) { VIndex = 0 };
                        }
                    }

                    cData = chunk.Instructions[c].CustomData;
                    if (cData != null)
                    {
                        cData.WrittenOpCode = op;
                    }
...
local ToNumber = tonumber;";
        ...
        vm += "return Chunk;end;";
        vm += settings.PreserveLineInfo ? Strings.VMP2_LI : Strings.VMP2;
        ...
        vm += settings.PreserveLineInfo ? Strings.VMP3_LI : Strings.VMP3;
